...
void TestCustomCapture::startRecording() {
	std::function<void()> func = [this]() {
		audioInput = new QAudioInput(audioFormat,nullptr);
		QByteArray buffer("");
		streamIn = audioInput->start();
        while (!stop_custom_audio_sender) {

            //if (streamIn) qDebug() << "1"<< streamIn->readAll();

            buffer.append(streamIn->readAll());
            if (buffer.size() >= buffer_size) {
                QByteArray nowData = buffer.mid(0, buffer_size);
                buffer.remove(0, buffer_size);
                frame->data = nowData.data();
