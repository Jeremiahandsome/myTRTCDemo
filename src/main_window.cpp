...
void MainWindow::on_btnStartBgmMix_clicked(){
	myITRTCAudioFrameCallback* callback = new myITRTCAudioFrameCallback;
	getTRTCShareInstance()->setAudioFrameCallback(callback);

	TRTCAudioFrameCallbackFormat* callbackFormat = new TRTCAudioFrameCallbackFormat;
	callbackFormat->sampleRate = 48000;
	callbackFormat->channel = 1;
	callbackFormat->samplesPerCall = 480;
	callbackFormat->mode = TRTCAudioFrameOperationModeReadWrite;
	getTRTCShareInstance()->setLocalProcessedAudioFrameCallbackFormat(callbackFormat);
}
...
void MainWindow::updateModuleButtonStatus(bool isEnteredRoom){
    ...
    ui_mainwindow_->btnStartBgmMix->setEnabled(isEnteredRoom);
    ui_mainwindow_->pushButtonCustomMessage->setEnabled(isEnteredRoom);
    ui_mainwindow_->btnCustomAudioRender->setEnabled(!isEnteredRoom);
    updateDynamicTextUI();
}
...
void MainWindow::startAudioRender() {
	std::function<void()> func = [this]() {
		...
		if (!outputDeviceInfo.isFormatSupported(outputFormat)) {
            outputFormat = outputDeviceInfo.nearestFormat(outputFormat);
		}

        QIODevice* streamOut;
        audioOutput = new QAudioOutput(outputFormat);
		streamOut = audioOutput->start();
		while (!stopCustomAudioRender) {
			getTRTCShareInstance()->getCustomAudioRenderingFrame(renderAudioFrame);
            streamOut->write(renderAudioFrame->data, buffer_size);
			QThread::msleep(20);
		}
		audioOutput->stop();
	};
...
void MainWindow::on_btnCustomAudioRender_clicked()
{
    if (!isCustomAudioRenderOpen) {
        //getTRTCShareInstance()->muteAllRemoteAudio(true);
        stopCustomAudioRender = false;
        getTRTCShareInstance()->enableCustomAudioRendering(true);

		renderAudioFrame = new TRTCAudioFrame;
        renderAudioFrame->audioFormat = TRTCAudioFrameFormatPCM;
        renderAudioFrame->sampleRate = 48000;
        renderAudioFrame->channel = 1;
        renderAudioFrame->data = new char[1920];
        renderAudioFrame->length = buffer_size;
        
		//getTRTCShareInstance()->getCustomAudioRenderingFrame(renderAudioFrame);
        //startAudioRender();
    }
    else {
        stopCustomAudioRender = true;
        getTRTCShareInstance()->enableCustomAudioRendering(false);
    ...
    isCustomAudioRenderOpen = !isCustomAudioRenderOpen;
    updateDynamicTextUI();
}
...
void MainWindow::updateDynamicTextUI() {
    ...
    if (isCustomAudioRenderOpen) {
        ui_mainwindow_->btnCustomAudioRender->setText(tr("关闭音频自定义渲染", "dynamic"));
    }
    else {
        ui_mainwindow_->btnCustomAudioRender->setText(tr("开启音频自定义渲染", "dynamic"));
    }
}